 ...
 * Copyright (C) 2016-2017, IS2T - EDC compliance and optimizations.
...
public abstract class SHA2 extends DigestBase {
	...
	private void i2bBig(int[] in, int inOfs, byte[] out, int outOfs, int len) {
		len += outOfs;
		while (outOfs < len) {
			writeBInt(out, outOfs, in[inOfs++]);
			outOfs += 4;
		}
	}

	// Store one 32-bit value into out[outOfs..outOfs+3] in big endian order.
	private void i2bBig4(int val, byte[] out, int outOfs) {
		writeBInt(out, outOfs, val);
	}

	private static void writeBInt(byte[] byteArray, int offset, int value) {
		byteArray[offset] = int3(value);
		byteArray[offset + 1] = int2(value);
		byteArray[offset + 2] = int1(value);
		byteArray[offset + 3] = int0(value);
	}
	...
	private void b2iBig64(byte[] in, int inOfs, int[] out) {
		for (int i = 0; i < 16; i++, inOfs+=4) {
			out[i] = readBInt(in, inOfs);
		}
	}

	private static int readBInt(byte[] byteArray, int offset) {
		return (((byteArray[offset]) << 24) | ((byteArray[offset + 1] & 0xff) << 16)
